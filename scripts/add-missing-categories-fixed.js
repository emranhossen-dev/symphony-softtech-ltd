...
async function addMissingCategories() {
  try {
    ...
    for (const categoryData of requiredCategories) {
      ...
      // Check if category exists by name (for Government Courses)
      const existingByName = await prisma.category.findFirst({
        where: { name: categoryData.name }
      });

      if (existingByName) {
        // Update the existing category to have the correct slug
        ...
        console.log(`🔄 Updated category: ${updated.name} (new slug: ${updated.slug})`);
        continue;
      }
