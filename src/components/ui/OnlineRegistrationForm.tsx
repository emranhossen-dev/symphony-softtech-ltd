...
import { User, Phone, Mail, MapPin, CreditCard, CheckCircle, AlertCircle, Monitor, Clock } from "lucide-react";
import toast from 'react-hot-toast';

// Validation schema for Online Course Registration
const onlineRegistrationSchema = z.object({
  ...
  preferredBatchTime: z.string().min(1, "Please select your preferred batch time"),
  agreeToTerms: z.boolean().refine((val) => val === true, "You must agree to the terms and conditions"),
});

type OnlineRegistrationFormData = z.infer<typeof onlineRegistrationSchema>;

interface OnlineRegistrationFormProps {
  courseName?: string;
  onSuccess?: (data: OnlineRegistrationFormData) => void;
  className?: string;
}

const OnlineRegistrationForm = ({ courseName = "", onSuccess, className = "" }: OnlineRegistrationFormProps) => {
  ...
  const onSubmit = async (data: OnlineRegistrationFormData) => {
  ...
  const batchTimes = [
    { value: "morning", label: "Morning Batch (9:00 AM - 12:00 PM)" },
    { value: "afternoon", label: "Afternoon Batch (2:00 PM - 5:00 PM)" },
    { value: "evening", label: "Evening Batch (7:00 PM - 10:00 PM)" },
    { value: "weekend", label: "Weekend Batch (Saturday & Sunday)" },
  ];

  const onlineCourses = [
    "Live Full Stack Bootcamp",
    "Live Data Science Bootcamp",
    "Live Mobile Development",
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Join our interactive live classes with expert instructors
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="">Select a live online course</option>
          ...
          <div>
            <label htmlFor="preferredBatchTime" className="block text-sm font-medium text-gray-700 mb-2">
              Preferred Batch Time <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <Clock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
              <select
                id="preferredBatchTime"
                {...register("preferredBatchTime")}
                className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-300 appearance-none text-gray-900 focus:text-orange-600 ${
                  errors.preferredBatchTime ? "border-danger focus:ring-danger/20" : "border-gray-300"
                }`}
              >
                <option value="">Select your preferred batch time</option>
                {batchTimes.map((batch) => (
                  <option key={batch.value} value={batch.value}>{batch.label}</option>
                ))}
              </select>
              {errors.preferredBatchTime && (
                <div className="flex items-center gap-2 mt-1">
                  <AlertCircle className="w-4 h-4 text-danger flex-shrink-0" />
                  <span className="text-danger text-sm">{errors.preferredBatchTime.message}</span>
                </div>
              )}
            </div>
          </div>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                <Monitor className="inline-block ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
              </>
            )}
          </button>
        </form>
